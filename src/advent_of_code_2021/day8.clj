...
(defn process-input-simple [input]
  (-> input
...
(defn process-input-all [input]
  (-> input
      (clojure.string/split #"\|")))

(defn identify-simple [line]
  (map
   (fn [e] (case (count e)
            2 1
            4 4
...
(defn deduce-numbers [numbers]
  (let [freq (frequencies (clojure.string/replace numbers #" " ""))
        sorted (sort-by count (clojure.string/split numbers #"\ "))
        two (first sorted)
        four (nth sorted 2)
        elements (into {} (map (fn [[el freq]]
                                 (case freq
                                   8 (if (clojure.string/includes? two (str el))
                                       ["e1" el]
                                       ["e6" el])
                                   9 ["e2" el]
                                   7 (if (not (clojure.string/includes? four (str el)))
                                       ["e3" el]
                                       ["e7" el])
                                   4 ["e4" el]
                                   6 ["e5" el]))
                               freq))
        new-numbers-chars (map #(map elements %) [["e1" "e2" "e3" "e4" "e5" "e6"]
                                                  ["e1" "e2"]
                                                  ["e1" "e3" "e4" "e6" "e7"]
                                                  ["e1" "e2" "e3" "e6" "e7"]
                                                  ["e1" "e2" "e5" "e7"]
                                                  ...
                                                  ["e1" "e2" "e3" "e5" "e6" "e7"]])
        new-numbers-str (map #(apply str %) new-numbers-chars)
        new-numbers-map (zipmap new-numbers-str (range 0 10))]
    new-numbers-map))
  ...
  (let [freq (frequencies (clojure.string/replace numbers #" " ""))
        ...
        elements (into {} (map (fn [[el freq]]
                                 (case freq
                                   ...
                                   6 ["e5" (str el)]))
                               freq))]
    {(sort (first (filter #(not (clojure.string/includes? % (elements "e7"))) (take 3 (drop 6 sorted))))) 0
     (sort (first sorted)) 1
     (sort (first (filter #(clojure.string/includes? % (elements "e4")) (take 3 (drop 3 sorted))))) 2
     (sort (first (filter #(clojure.string/includes? % (elements "e1"))
                          (filter #(clojure.string/includes? % (elements "e2")) (take 3 (drop 3 sorted)))))) 3
     (sort (nth sorted 2)) 4
     (sort (first (filter #(clojure.string/includes? % (elements "e5")) (take 3 (drop 3 sorted))))) 5
     (sort (first (filter #(not (clojure.string/includes? % (elements "e1"))) (take 3 (drop 6 sorted))))) 6
     (sort (second sorted)) 7
     (sort (last sorted)) 8
     (sort (first (filter #(not (clojure.string/includes? % (elements "e4"))) (take 3 (drop 6 sorted))))) 9}))

(defn identify-all [[numbers input]]
  (let [number-mapping (deduce-numbers numbers)
        split (clojure.string/split (clojure.string/trim input) #" ")
        sorted (map sort split)]
    (map number-mapping sorted)))

(defn count-not-nil [lines]
  (->> lines
       (map #(filter (complement nil?) %))
       (map count)))

(defn count-simple-numbers [lines]
  (->> lines
       clojure.string/split-lines
       (map process-input-simple)
       (map identify-simple)
       (count-not-nil)
       (apply +)))

(defn count-all-numbers [lines]
  (->> lines
       clojure.string/split-lines
       (map process-input-all)
       (map identify-all) 
       (map #(apply str %))
